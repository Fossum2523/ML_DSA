...
#(  parameter DLEN = 23, A_HLEN = 12, T_HLEN = 10, HLEN = 8, BIT_LEN = 23)
    ...
    reg [63:0]      KEY_data_a;
    reg [63:0]      KEY_data_b;
    reg [9 - 1:0]   KEY_addr_a;
    reg [9 - 1:0]   KEY_addr_b;
    reg             KEY_en_a;
    reg             KEY_en_b;
    reg             KEY_we_a;
    reg             KEY_we_b;
    ...
    always @(*) begin
        ...
        case (kk_sub_sel_1)
            ...
            2'd1: kk_sub_1_in = 64'd0;         //u mem
            2'd2: kk_sub_1_in = 64'd0;         //w mem
    ...
    always @(*) begin
        case (kk_sub_sel_3)
            ...
            2'd3: kk_sub_3_in = 64'd0;              //y_gen_index
    ...
    Data_Mem DM(
        ...
        /*---KEY---*/
        .KEY_data_a(KEY_data_a),
        .KEY_data_b(KEY_data_b),
        .KEY_addr_a(KEY_addr_a),
        .KEY_addr_b(KEY_addr_b),
        .KEY_en_a(KEY_en_a),
        .KEY_en_b(KEY_en_b),
        .KEY_we_a(KEY_we_a),
        .KEY_we_b(KEY_we_b),
    ...
    //     sha_mux_sel = 3'd0;
    //     case (ctrl_sign)
    //         {KeyGen,6'd1}:begin // Enocder s1 to s1_pack and store in KEY MEM
    //             sha_mux_sel = AG_4_addr_a >> 1
    //         end
    //     endcase
    // end
    ...
    always @(*) begin
        ...
        KEY_we_b   = 1'd0; 
        case (ctrl_sign)
            {KeyGen,6'd4}:begin // Enocder s1 to s1_pack and store in KEY MEM
                KEY_data_a = ENC_dout;
                KEY_addr_a = AG_3_addr_a;//
                KEY_en_a   = ENC_valid_o;
                KEY_we_a   = ENC_valid_o;
            end
            {KeyGen,6'd5}:begin // Enocder s2 to s2_pack and store in KEY MEM
                KEY_data_a = ENC_dout;
                KEY_addr_a = AG_3_addr_a;//
                KEY_en_a   = ENC_valid_o;
                KEY_we_a   = ENC_valid_o;
            end
            {KeyGen,6'd7}:begin // Enocder t to t1_pack and store in KEY MEM
                KEY_data_a = ENC_dout;
                KEY_addr_a = AG_3_addr_a;
                KEY_en_a   = ENC_valid_o;
                KEY_we_a   = ENC_valid_o;
            ...
            {KeyGen,6'd8}:begin 
                //Read pk(p and t1_pack) from KEY MEM into Keccak to generate tr
                KEY_addr_a = AG_1_addr_a[7:0];
                KEY_en_a  =  (main_mem_sel[0]) & AG_1_addr_en;
                //Store the tr generated by Keccak into KEY MEM
                if(AG_4_addr_a[0])
                    KEY_data_b = seed_out[127:64];
                else
                    KEY_data_b = seed_out[63:0];
                KEY_addr_b = AG_4_addr_a[2:0];
                KEY_en_b  =  AG_4_addr_en;
                KEY_we_b  =  AG_4_addr_en;
            end
        endcase
    end
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            {KeyGen,6'd4}: begin //store 64bits s1 data to mem
                ...
                AG_3_star_addr   = 12'd180;
                AG_3_last_addr   = 12'd228;//speical to add 1 to make wait ENC_valid_o high
            end 
            {KeyGen,6'd5}: begin //store 64bits s2 data to mem
                AG_3_addr_adder  = {1'b0,ENC_valid_o};
                AG_3_star_addr   = 12'd228;
                AG_3_last_addr   = 12'd276;//speical to add 1 to make wait ENC_valid_o high
            end 
            {KeyGen,6'd7}: begin //store 64bits t1 data to mem
                AG_3_addr_adder  = {1'b0,ENC_valid_o};
                AG_3_star_addr   = 12'd4;
                AG_3_last_addr   = 12'd164;//speical to add 1 to make wait ENC_valid_o high
            end 
        ...
        case (ctrl_sign)
            {KeyGen,6'd9}:begin
                if(~main_mem_sel[0])
                    ...
                    data_out = t1_pack_q_a;
            end
            {KeyGen,6'd10}:begin
                case (main_mem_sel)
                    4'd0: data_out = seed_q_a;
                    4'd1: data_out = seed_q_a;
                    4'd2: data_out = tr_q_a;
                    4'd3: data_out = s1_pack_q_a;
                    4'd4: data_out = s2_pack_q_a;
                    4'd5: data_out = t0_pack_q_a;
                endcase
            end
        endcase
    end
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            {KeyGen,6'd9}:begin
                o_last = AG_1_done & main_mem_sel[0];
            end
            {KeyGen,6'd10}:begin
                o_last = AG_1_done & main_mem_sel == 4'd5;
            end
        endcase
    end
    /*---Outside---*/ //------------------------------------------end
