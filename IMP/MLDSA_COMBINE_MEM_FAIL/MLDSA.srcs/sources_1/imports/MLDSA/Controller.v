...
module Controller
    ...
    always @(*) begin
        case (curr_state_KeyGen)
            ...
            STAGE_4: begin
                if(Econder_done_tmp && keccak_done)   
            ...
            STAGE_8: begin
                if(Econder_done_tmp && keccak_done_tmp)   
    ...
    always @ (posedge clk) begin 
        if (reset)                                                                                                                                                   
            s_mem_cnt <= 4'd0;
        else if((curr_state == {KeyGen,STAGE_2} | curr_state == {KeyGen,STAGE_3}) && next_element)
            s_mem_cnt <= s_mem_cnt + 1'b1;
    end
    ...
    always @ (posedge clk) begin 
        if (reset)                                                                                                                                                   
            A_mem_cnt <= 4'd0;
        else if(curr_state == {KeyGen,STAGE_4} && next_element)
            A_mem_cnt <= A_mem_cnt + 1'b1;
    end
