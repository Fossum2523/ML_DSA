...
//                               /*(mode == G)?*/ {in[575:0] ^ out[1599:1599-575], out[1599-576:0]} ) : out; // need update
    ...
    always @ (posedge clk)
      if (reset)
        ...
        out <= (mode == XOF)? {out[1599:1344], in[1343:0] ^ out[1343:0]}: //204 G sahke128
               (mode == KDF)? {out[1599:1088], in[1087:0] ^ out[1087:0]}:
               (mode == PRF)? {out[1599:1088], in[1087:0] ^ out[1087:0]}:
               (mode == H)? {out[1599:1088], in[1087:0] ^ out[1087:0]}:   //204 H sahke256
               /*(mode == G)?*/ {out[1599:576], in[575:0] ^ out[575:0]};
