module Sampler(
    ...
    output  [22:0]      cj,             // Write data ci to Mem
    ...
    always @(posedge clk or negedge resetn) begin
        if(!resetn)
            ...
            sampler_buffer <= mode == S_mode    ? {256'd0,8'd0,sampler_buffer[1087:8]}:
                              ...
                              mode == SIB_mode & SIB_load_H & SIB_o_done ? {256'd0,8'd0,sampler_buffer[1087:8]}: sampler_buffer;
    end
        ...
        if (!resetn)
            ...
            if(mode == SIB_mode)
                if(~SIB_load_H)
                    ...
                    shake_cnt <= shake_cnt + SIB_o_done; // Reset or increment
            else
