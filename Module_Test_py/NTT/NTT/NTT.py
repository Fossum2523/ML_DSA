...
# NTT
def NTT(w):
    if os.path.exists("NTT_stage_gold0.txt"):
        os.remove("NTT_stage_gold0.txt")
    ...
    while length >= 1:
        ...
        while start < 256:
            k = k + 1
            zeta = zeta_list[k]
            # with open(f"zeta_mem_{cnt}.txt", "a") as file:
            #     file.write(f"{hex(zeta)}\n")
            # print(hex(zeta))
            # with open("NTT_stage_gold.txt", "a") as file:  # "w" 代表寫入模式，會覆蓋原內容
            #         file.write(f"/***stage_{cnt}***/\n")
            for j in range(start, start + length):
                t = (zeta * w_hat[j + length]) % ML_DSA["q"]
                temp0 = w_hat[j]
                temp1 = w_hat[j + length]
                w_hat[j + length] = (w_hat[j] - t) % ML_DSA["q"]
                w_hat[j] = (w_hat[j] + t) % ML_DSA["q"]
                if(length == 128 and j == start):
                     print("----------------------")
                     print(zeta)
                     print(t)
                    #  print(temp0)
                    #  print(temp1)
                     print(w_hat[j])
                     print(w_hat[j + length])
                     
                with open("NTT_stage_gold0.txt", "a") as file:  # "w" 代表寫入模式，會覆蓋原內容
                    file.write(f"w_hat[{j}] = {w_hat[j]}\n")
                    file.write(f"w_hat[{j + length}] = {w_hat[j + length]}\n")
            start += 2 * length
        for i in range(256):
            with open("NTT_stage_gold.txt", "a") as file:  # "w" 代表寫入模式，會覆蓋原內容
                file.write(f"{w_hat[i]}\n")
        length //= 2
        cnt = cnt + 1
