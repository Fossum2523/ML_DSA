...
#(  parameter DLEN = 23, A_HLEN = 12, T_HLEN = 10, Y_HLEN = 10, HLEN = 8, BIT_LEN = 23)
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd4}:begin //store p from MLDSA_in_B (stage_4-1)
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd12}:begin //NTT c -> c_hat  and clean to 0 for gen c
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd7}:begin //PWM W^ = A^ * y^ store w^ in t MEM
                if(~PWM_index[0])begin
                    t_addr_a = AG_4_addr_a;
                    t_addr_b = AG_4_addr_a + 1'b1;
                    t_en_a   = AG_4_addr_en;
            ...
            {SignGen,6'd8}:begin // INTT W^, take the w^ data from t MEM
                t_addr_a = {NTT_index, AG_2_addr_a[7:0]};
                t_addr_b = {NTT_index, AG_2_addr_a[7:0]} + 1'b1;
                t_en_a   = AG_2_addr_en;
                t_en_b   = AG_2_addr_en;
            end
            {SignGen,6'd13}:begin // PWM <<cs1>>^ = c^ * s1^ and store <<cs1>>^ here
                t_data_a = PWM_out_a;
                t_data_b = PWM_out_b;
                t_addr_a = AG_4_addr_a - 2'd2;
                t_addr_b = AG_4_addr_a - 2'd1;
            ...
            {SignGen,6'd14}:begin // INTT <<cs1>>^, send out <<cs1>>^ data
                t_addr_a = {NTT_index, AG_2_addr_a[7:0]};
                t_addr_b = {NTT_index, AG_2_addr_a[7:0]} + 1'b1;
                t_en_a   = AG_2_addr_en;
                t_en_b   = AG_2_addr_en;
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd16}:begin //PWM w_cs2 = 𝐰 − ⟨⟨𝑐𝐬2⟩⟩ and LowBits(w_cs2), store Hightbit(𝐰 − ⟨⟨𝑐𝐬2⟩⟩) here
                t0_data_a = {17'd0,Decomposer_w1_a};
                t0_data_b = {17'd0,Decomposer_w1_b};
                t0_addr_a = AG_4_addr_a - 2'd2;
                t0_addr_b = AG_4_addr_a - 2'd1;
                t0_en_a   = AG_4_data_valid;
                t0_en_b   = AG_4_data_valid;
                t0_we_a   = AG_4_data_valid;
                t0_we_b   = AG_4_data_valid;
            end
            {SignGen,6'd19}:begin //send out Hightbit(𝐰 − ⟨⟨𝑐𝐬2⟩⟩) make hint
                t0_addr_a = AG_4_addr_a;
                t0_addr_b = AG_4_addr_b;
                t0_en_a   = AG_4_addr_en;
                t0_en_b   = AG_4_addr_en;
            end
        endcase
    end
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd19}:begin // store hint
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd13}:begin //PWM <<cs1>> = c^ * s1^ and send c_hat data to PWM
                c_hat_addr_a = AG_4_addr_a[7:0];
                c_hat_addr_b = AG_4_addr_b[7:0];
                c_hat_en_a   = AG_4_addr_en;
                c_hat_en_b   = AG_4_addr_en;
            end
            {SignGen,6'd14}:begin //PWM <<cs2>> = c^ * s2^ and send c_hat data to PWM
                c_hat_addr_a = AG_4_addr_a[7:0];
                c_hat_addr_b = AG_4_addr_b[7:0];
                c_hat_en_a   = AG_4_addr_en;
                c_hat_en_b   = AG_4_addr_en;
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd13}:begin //PWM <<cs1>> = c^ * s1^ and send s1^ data to PWM
                temp_0_addr_a = AG_4_addr_a;
                temp_0_addr_b = AG_4_addr_b;
                temp_0_en_a   = AG_4_addr_en;
                temp_0_en_b   = AG_4_addr_en;
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd14}:begin //PWM <<cs2>> = c^ * s2^ and send s2^ data to PWM
    ...
    always @(*) begin
        ...
        temp_2_we_b   = 1'd0;
        case (ctrl_sign)
            {SignGen,6'd5}:begin //NTT t0 and temp here
    ...
    always @(*) begin
        ...
        temp_3_we_b   = 1'd0;
        case (ctrl_sign)
            {SignGen,6'd6}:begin //NTT y and temp here
            ...
            {SignGen,6'd17}:begin //NTT y and temp here
                temp_3_data_a = NTT_out_u;
                temp_3_data_b = NTT_out_d;
                temp_3_addr_a = {NTT_index,NTT_addr_u};
                temp_3_addr_b = {NTT_index,NTT_addr_d};
    ...
    always @(*) begin
        ...
        temp_4_we_b   = 1'd0;
        case (ctrl_sign)
            {SignGen,6'd15}:begin //PWM z = y + <<cs1>> and store z here
            ...
            {SignGen,6'd21}:begin // Enocder z and send out
    ...
    always @(*) begin
        ...
        temp_5_we_b   = 1'd0;
        case (ctrl_sign)
            {SignGen,6'd14}:begin // INTT <<cs1>> and store <<cs1>>
                temp_5_data_a = NTT_out_u;
                temp_5_data_b = NTT_out_d;
                temp_5_addr_a = {NTT_index,NTT_addr_u};
                temp_5_addr_b = {NTT_index,NTT_addr_d};
                temp_5_en_a   = NTT_out_ready;
                temp_5_en_b   = NTT_out_ready;
                temp_5_we_a   = NTT_out_ready;
                temp_5_we_b   = NTT_out_ready;
            end
            {SignGen,6'd15}:begin
                temp_5_addr_a = AG_4_addr_a;
                temp_5_addr_b = AG_4_addr_b;
                temp_5_en_a   = AG_4_addr_en;
                temp_5_en_b   = AG_4_addr_en;
            end
            {SignGen,6'd18}:begin //INTT INTT(<<ct0>>^ )0 and temp ct0 here
                temp_5_data_a = NTT_out_u;
                temp_5_data_b = NTT_out_d;
                temp_5_addr_a = {NTT_index,NTT_addr_u};
                temp_5_addr_b = {NTT_index,NTT_addr_d};
            ...
            {SignGen,6'd19}:begin //PWM (𝐰 − ⟨⟨𝑐𝐬2⟩⟩) + <<ct0>>, sned out ct0 data 
                temp_5_addr_a = AG_4_addr_a;
                temp_5_addr_b = AG_4_addr_b;
                temp_5_en_a   = AG_4_addr_en;
                temp_5_en_b   = AG_4_addr_en;
    ...
    always @(*) begin
        ...
        temp_6_we_b   = 1'd0;
        case (ctrl_sign)
            {SignGen,6'd15}:begin // INTT <<cs2>>^ and store <<cs2>>
                temp_6_data_a = NTT_out_u;
                temp_6_data_b = NTT_out_d;
                temp_6_addr_a = {NTT_index,NTT_addr_u};
                temp_6_addr_b = {NTT_index,NTT_addr_d};
                temp_6_en_a   = NTT_out_ready;
                temp_6_en_b   = NTT_out_ready;
                temp_6_we_a   = NTT_out_ready;
                temp_6_we_b   = NTT_out_ready;
            end
            {SignGen,6'd16}:begin //PWM w_cs2 = 𝐰 − ⟨⟨𝑐𝐬2⟩⟩ and LowBits(w_cs2)
            ...
            {SignGen,6'd17}:begin //PWM <<ct0>>^ = c^ * t0^ and store <<ct0>>^
                temp_6_data_a = PWM_out_a;
                temp_6_data_b = PWM_out_b;
                temp_6_addr_a = AG_4_addr_a - 2'd2;
                temp_6_addr_b = AG_4_addr_a - 2'd1;
                temp_6_en_a   = AG_4_data_valid;
                temp_6_en_b   = AG_4_data_valid;
                temp_6_we_a   = AG_4_data_valid;
                temp_6_we_b   = AG_4_data_valid;
            end
            {SignGen,6'd18}:begin // INTT(<<ct0>>^ ), send out <<ct0>>^ data
                temp_6_addr_a = {NTT_index, AG_2_addr_a[7:0]};
                temp_6_addr_b = {NTT_index, AG_2_addr_a[7:0]} + 1'b1;
                temp_6_en_a   = AG_2_addr_en;
                temp_6_en_b   = AG_2_addr_en;
            end
        endcase
    end
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd14}:begin // PWM <<cs2>> = c^ * s2^ and store <<cs2>>^ here
                PWM_temp_data_a = PWM_out_a;
                PWM_temp_data_b = PWM_out_b;
                PWM_temp_addr_a = AG_4_addr_a - 2'd2;
                PWM_temp_addr_b = AG_4_addr_a - 2'd1;
            ...
            {SignGen,6'd15}:begin // INTT <<cs2>>^, send out <<cs2>>^ data
            ...
            {SignGen,6'd16}:begin //w-cs2 = 𝐰 − ⟨⟨𝑐𝐬2⟩⟩) store here
                PWM_temp_data_a = PWM_out_a;
                PWM_temp_data_b = PWM_out_b;
                PWM_temp_addr_a = AG_4_addr_a - 2'd2;
                PWM_temp_addr_b = AG_4_addr_a - 2'd1;
            ...
            {SignGen,6'd19}:begin //PWM (𝐰 − ⟨⟨𝑐𝐬2⟩⟩) + <<ct0>>, sned out w-cs2 data 
                PWM_temp_addr_a = AG_4_addr_a;
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd5},        //Gen A and take the seed from mem to keccak
            {SignGen,6'd6}: begin  //Gen A and take the seed from mem to keccak
                AG_1_addr_adder  = 2'd1;
                AG_1_star_addr   = 12'd8;
                AG_1_last_addr   = 12'd11;
            end 
            {SignGen,6'd10}: begin  //Gen pk and take the seed p from mem to keccak
                AG_1_pasue  = sha_buffer_full;
                if(~main_mem_sel[0])begin
                    AG_1_addr_adder  = 2'd1;
                    AG_1_star_addr   = 12'd0;
            ...
            {SignGen,6'd11}: begin  //Gen c and take the c_tilde from mem to keccak
                AG_1_addr_adder  = 2'd1;
                AG_1_star_addr   = 12'd0;
                AG_1_last_addr   = 12'd3;
            end
            {SignGen,6'd16}: begin  //Gen y and take p'' from seed mem to keccak
                AG_1_addr_adder  = 2'd1;
                AG_1_star_addr   = 12'd0;
                AG_1_last_addr   = 12'd7;
            end 
            {SignGen,6'd20}: begin  //send out c_tilde from c_tilde MEM
                AG_1_addr_adder  = 2'd1;
                AG_1_star_addr   = 12'd0;
                AG_1_last_addr   = 12'd3;
            end 
            {SignGen,6'd22}: begin  //send out hint_packed from s1_packe MEM
                AG_1_addr_adder  = 2'd1;
                AG_1_star_addr   = 12'd0;
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd12}: begin  // NTT c -> c_hat
                AG_2_addr_adder  = 2'd1;
                AG_2_star_addr   = 12'd0;
                AG_2_last_addr   = 12'd127;
            end
            ...
            {SignGen,6'd21}: begin //take 18 bits z data to encoder and change it to 64bits
                AG_2_addr_adder  = 2'd2;
                AG_2_star_addr   = 12'd0;
                AG_2_last_addr   = 12'd1022;
            end  
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd9}: begin //store 64bits w1 data to mem
                AG_3_addr_adder  = {1'b0,ENC_valid_o};
                AG_3_star_addr   = 12'd0;
                AG_3_last_addr   = 12'd96;//speical to add 1 to make wait ENC_valid_o high
            end 
            {SignGen,6'd19}: begin //store 64bits h_pack data to mem
                AG_3_addr_adder  = {1'b0,MH_hint_valid_o};
                AG_3_star_addr   = 12'd0;
                AG_3_last_addr   = 12'd10;
            end 
            {SignGen,6'd21}: begin //store 64bits z data to mem
                AG_3_addr_adder  = {1'b0,ENC_valid_o};
                AG_3_star_addr   = 12'd0;
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd10}: begin  //Gen c_tilde by keccak and take data to c_tilde mem 
                AG_4_addr_adder  = 2'd2;
                AG_4_star_addr   = 12'd0;
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd15}:begin //PWM z = y + <<cs1>>
                PWM_mode  = ADD_VECTOR;
                PWM_in_a1 = temp_5_q_a; //<<cs1>>
                PWM_in_a2 = y_q_a;      //y
                PWM_in_b1 = temp_5_q_b; //<<cs1>>
                PWM_in_b2 = y_q_b;      //y
            end
            {SignGen,6'd16}:begin // PWM w_cs2 = 𝐰 − ⟨⟨𝑐𝐬2⟩⟩ and LowBits(w_cs2)
                PWM_mode  = ADD_VECTOR;
                PWM_in_a1 = 23'd8380417 - temp_6_q_a; //<<cs2>>
                PWM_in_a2 = temp_3_q_a; //w
                PWM_in_b1 = 23'd8380417 - temp_6_q_b; //<<cs2>>
            ...
            {SignGen,6'd19}:begin //PWM (𝐰 − ⟨⟨𝑐𝐬2⟩⟩) + <<ct0>>
                PWM_mode  = ADD_VECTOR;
                PWM_in_a1 = temp_5_q_a; //<<ct0>>
                PWM_in_a2 = PWM_temp_q_a; //w_cs2
                PWM_in_b1 = temp_5_q_b; //<<ct0>>
    ...
    Decomposer Decomposer_(
        ...
        .r0_out_a(Decomposer_w0_a),  //LowBit   18 bits
        .r1_out_a(Decomposer_w1_a),  //HighBit  6 bits

        //protocol 1
        .r_in_b(Decomposer_in_b),
        .r0_out_b(Decomposer_w0_b),  //LowBit   18 bits
        .r1_out_b(Decomposer_w1_b)   //HighBit  6 bits
    );
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            {SignGen,6'd9}:begin //Decomposer w1 =Decomp(w)
                Decomposer_in_a = temp_3_q_a;
                Decomposer_in_b = temp_3_q_b;
            end
            {SignGen,6'd16}:begin // PWM w_cs2 = 𝐰 − ⟨⟨𝑐𝐬2⟩⟩ and LowBits(w_cs2) and HighBits(w_cs2)
                Decomposer_in_a = PWM_out_a;
                Decomposer_in_b = PWM_out_b;
            end
            {SignGen,6'd19}:begin //Highits(𝐰 − ⟨⟨𝑐𝐬2⟩⟩ + <<ct0>>)
                Decomposer_in_a = PWM_out_a;
                Decomposer_in_b = PWM_out_b;
            end
        endcase
    ...
    always @(posedge clk) begin
        ...
        else if(ctrl_sign == {SignGen,6'd16} && AG_4_data_valid)begin
    ...
    always @(posedge clk) begin
        ...
        else if(ctrl_sign == {SignGen,6'd18} && NTT_out_ready)begin
    ...
    assign hint_a = ctrl_sign == {SignGen,6'd19} ? t0_q_a[5:0] != Decomposer_w1_a: 1'b0;
    assign hint_b = ctrl_sign == {SignGen,6'd19} ? t0_q_b[5:0] != Decomposer_w1_b: 1'b0;
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            {SignGen,6'd19}:begin
                MH_sec_lvl = 3'd2;
                MH_poly0_ie = {{18'd0,Decomposer_w1_b},{18'd0,Decomposer_w1_a}};
                MH_poly1_ie = {{18'd0,t0_q_b[5:0]},{18'd0,t0_q_a[5:0]}};
    ...
    always @(*) begin
        ...
        case (ctrl_sign)
            ...
            {SignGen,6'd20}:begin
                MLDSA_data_out = c_tilde_q_a;
            end
            {SignGen,6'd21}:begin
                MLDSA_data_out = ENC_dout;
            end
            {SignGen,6'd22}:begin
                MLDSA_data_out = s1_pack_q_a;
            end
        endcase
    end
